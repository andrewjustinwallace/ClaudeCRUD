...
const NewHiresGrid = () => {
  ...
  useEffect(() => {
    if (selectedCompany) {
      loadNewHiresByCompany(selectedCompany);
    }
    // else {
    //   loadAllNewHires();
    // }
  ...
  const loadAllNewHires = async () => {
    try {
      const response = await axios.get(`${API_BASE_URL}/newhire`);
      setNewHires(response.data);
    } catch (error) {
      console.error("Failed to load new hires:", error);
    }
  };

  const loadNewHiresByCompany = async (companyId) => {
    try {
      const response = await axios.get(
        `${API_BASE_URL}/newhire/company/${companyId}`
      );
      setNewHires(response.data);
    } catch (error) {
      console.error("Failed to load new hires:", error);
